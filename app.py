...
def _run_flow():
    try:
        ...
        email, _pw, token = mail_handler.create_account()
        _session["email"] = email
        _session["token"] = token
        socketio.emit("status", {"type": "email", "msg": email})

        # Step 2: poll for verification code
        socketio.emit("status", {"type": "log", "msg": "Waiting for verification email (up to 3 min)..."})
        for event, value in mail_handler.poll_for_code(token):
            if event == "waiting":
                socketio.emit("status", {"type": "log", "msg": "No email yet, checking again..."})
            elif event == "code":
                socketio.emit("status", {"type": "code", "msg": value})
                socketio.emit("status", {"type": "log", "msg": "Done! Enter the code above in the Taco Bell app."})
                return

    except TimeoutError:
        socketio.emit("status", {"type": "error", "msg": "Timed out — no verification email arrived. Try again."})
